...
class JwtAuthorizerFactory extends AbstractResourceFactory<
...
describe('Integration Tests - Expression Evaluation', () => {
    ...
    it('should handle LITERAL policy', async () => {
        ...
        expect(resource).toBeDefined();
        expect(resource!.issuer).toBe(
            '${env:AUTH_ISSUER:https://auth.dev.local}'
        );
        expect(resource!.audience).toBe('${env:AUTH_AUD:naylence-node}');
        expect(resource!.requiredScopes).toBe(
            '${env:AUTH_SCOPES:fabric.read fabric.write}'
        );
    });
    ...
    it('should handle ERROR policy', async () => {
        ...
        try {
            ...
            expect((error as Error).message).toContain(
                'Expression found but evaluation is not allowed'
            );
        } finally {
            // Clean up
            delete (globalThis as any).process;
