...
describe('ExpressionEvaluator', () => {
    ...
    describe('isExpression', () => {
        it('should detect environment variable expressions', () => {
            expect(ExpressionEvaluator.isExpression('${env:TEST_VAR}')).toBe(
                true
            );
            expect(
                ExpressionEvaluator.isExpression('${env:TEST_VAR:default}')
            ).toBe(true);
            expect(
                ExpressionEvaluator.isExpression(
        ...
        it('should detect configuration expressions', () => {
            expect(ExpressionEvaluator.isExpression('${config:key}')).toBe(
                true
            );
            expect(
                ExpressionEvaluator.isExpression('${config:nested.key:default}')
            ).toBe(true);
        });

        it('should not detect non-expressions', () => {
            expect(ExpressionEvaluator.isExpression('plain string')).toBe(
                false
            );
            expect(ExpressionEvaluator.isExpression('${invalid:syntax')).toBe(
                false
            );
    ...
    describe('evaluate with LITERAL policy', () => {
        it('should return expressions unchanged', () => {
            ...
            expect(
                ExpressionEvaluator.evaluate('${config:key:default}', context)
            ).toBe('${config:key:default}');
        });
    });
    ...
    describe('evaluate with ERROR policy', () => {
        ...
        it('should not throw error for non-expressions', () => {
            ...
            expect(ExpressionEvaluator.evaluate('plain string', context)).toBe(
                'plain string'
            );
    ...
    describe('evaluate with EVALUATE policy', () => {
        it('should resolve environment variables from context', () => {
            ...
            expect(ExpressionEvaluator.evaluate('${env:PORT}', context)).toBe(
                '8080'
            );
            expect(
        ...
        it('should resolve configuration values', () => {
            ...
            expect(
                ExpressionEvaluator.evaluate('${config:simple}', context)
            ).toBe('value');
            expect(
                ExpressionEvaluator.evaluate('${config:database.host}', context)
            ).toBe('localhost');
            expect(
                ExpressionEvaluator.evaluate('${config:database.port}', context)
            ).toBe('5432');
        });
        ...
        it('should use config defaults when key not found', () => {
            ...
            expect(
                ExpressionEvaluator.evaluate(
                    '${config:missing.key:default_value}',
                    context
                )
            ).toBe('default_value');
        });
        ...
        it('should handle mixed expressions in single string', () => {
            ...
            const result = ExpressionEvaluator.evaluate(
                'https://${env:HOST}:${config:port}/api',
                context
            );
        ...
        it('should handle Node.js process.env when available', () => {
            ...
            expect(
                ExpressionEvaluator.evaluate('${env:NODE_ENV}', context)
            ).toBe('test');
            expect(ExpressionEvaluator.evaluate('${env:PORT}', context)).toBe(
                '3000'
            );
    ...
    describe('non-string values', () => {
        it('should return non-string values unchanged', () => {
            ...
            expect(ExpressionEvaluator.evaluate(undefined, context)).toBe(
                undefined
            );
    ...
    describe('configuration value resolution edge cases', () => {
        it('should throw error for required config value without default', () => {
            ...
            expect(() => {
                ...
                "Configuration key 'missing.required' is required but not defined"
            );
        });
        ...
        it('should handle nested config access for undefined properties', () => {
            ...
            expect(() => {
                ...
                "Configuration key 'database.host' is required but not defined"
            );
        });
    });
