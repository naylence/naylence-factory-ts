...
describe('ResourceConfigValidator', () => {
    ...
    describe('basic validation', () => {
        ...
        it('should fail validation for config without type field', () => {
            ...
            expect(result.errors[0].message).toBe(
                'Configuration must have a string "type" field'
            );
        });
    ...
    describe('nested object validation', () => {
        ...
        it('should fail validation for missing nested required fields', () => {
            ...
            const portError = result.errors.find(
                (e) => e.path === 'database.port'
            );
            const passwordError = result.errors.find(
                (e) => e.path === 'database.credentials.password'
            );
    ...
    describe('array validation', () => {
        ...
        it('should fail validation for incorrect array item types', () => {
            ...
            const hasArrayErrors = result.errors.some((e) =>
                e.path.includes('[')
            );
    ...
    describe('expression processing', () => {
        ...
        it('should process expressions in nested objects', () => {
            ...
            expect((result.config! as any).database.credentials.username).toBe(
                'prod_user'
            );
            expect((result.config! as any).database.credentials.password).toBe(
                'secret'
            ); // default used
