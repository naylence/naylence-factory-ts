...
describe('Expressions', () => {
  describe('env expressions', () => {
    ...
    it('should create environment variable expressions with defaults', () => {
      ...
      expect(Expressions.env('HOST', 'localhost')).toBe('${env:HOST:localhost}');
  ...
  describe('config expressions', () => {
    it('should create config expressions without defaults', () => {
      expect(Expressions.config('database.host')).toBe('${config:database.host}');
      expect(Expressions.config('api.version')).toBe('${config:api.version}');
    });

    it('should create config expressions with defaults', () => {
      expect(Expressions.config('database.port', '5432')).toBe('${config:database.port:5432}');
      expect(Expressions.config('timeout', '30')).toBe('${config:timeout:30}');
      expect(Expressions.config('flag', '')).toBe('${config:flag:}'); // empty default
    });

    it('should work with function syntax', () => {
      expect(config('test.key')).toBe('${config:test.key}');
      expect(config('test.key', 'default')).toBe('${config:test.key:default}');
    });
  });

  describe('literal values', () => {
    it('should return literal strings unchanged', () => {
      expect(Expressions.literal('https://api.example.com')).toBe('https://api.example.com');
  ...
  describe('aliases', () => {
    it('should provide environment alias for env', () => {
      ...
      expect(Expressions.environment('TEST', 'default')).toBe('${env:TEST:default}');
    });

    it('should provide setting alias for config', () => {
      expect(Expressions.setting('key')).toBe('${config:key}');
      expect(Expressions.setting('key', 'default')).toBe('${config:key:default}');
    });
  });
