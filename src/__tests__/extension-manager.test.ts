...
describe('ExtensionManager', () => {
  ...
  beforeEach(() => {
    manager = new ExtensionManager<TestResource, TestConfig>('test.factories', 'TestFactory');
  });
  ...
  describe('factory registration', () => {
    ...
    it('should register factory instance with undefined isDefault and priority', () => {
      ...
      class UndefinedPropsFactory implements ResourceFactory<TestResource, TestConfig> {
        ...
        isDefault?: boolean;
        dependencies?: string[];
        create = jest.fn().mockResolvedValue({ id: 'test', name: 'Test' });
  ...
  describe('default factory selection', () => {
    ...
    it('should return single default factory without logging', () => {
      const singleDefaultManager = new ExtensionManager<TestResource, TestConfig>('single.default', 'SingleDefaultFactory');
    ...
    it('should return null when no defaults exist', () => {
      const noDefaultManager = new ExtensionManager<TestResource, TestConfig>('empty', 'Empty');
  ...
  describe('static global registry', () => {
    ...
    it('should manage global factory registration', () => {
      ExtensionManager.registerGlobalFactory('TestFactory', 'TestA', TestFactoryA);
      ExtensionManager.registerGlobalFactory('TestFactory', 'TestB', TestFactoryB);

      const factory = ExtensionManager.getGlobalFactory<TestResource, TestConfig>('TestFactory', 'TestA');
      expect(factory).toBeInstanceOf(TestFactoryA);
    });

    it('should register global factory instances', () => {
      const instance = new TestFactoryA();
      ExtensionManager.registerGlobalFactoryInstance('TestFactory', instance);

      const retrieved = ExtensionManager.getGlobalFactory<TestResource, TestConfig>('TestFactory', 'TestA');
      expect(retrieved).toBe(instance);
    });

    it('should find best global defaults', () => {
      ExtensionManager.registerGlobalFactory('TestFactory', 'TestA', TestFactoryA);
      ExtensionManager.registerGlobalFactory('TestFactory', 'TestC', TestFactoryC);

      const best = ExtensionManager.getBestGlobalDefaultFactory<TestResource, TestConfig>('TestFactory');
      expect(best).not.toBeNull();
    ...
    it('should get extension manager for specific types', () => {
      const manager1 = ExtensionManager.getExtensionManager<TestResource, TestConfig>('test.group', 'TestFactory');
      const manager2 = ExtensionManager.getExtensionManager<TestResource, TestConfig>('test.group', 'TestFactory');
  ...
  describe('multiple defaults priority selection', () => {
    it('should select from multiple defaults and trigger sorting logic', () => {
      ...
      const multiManager = ExtensionManager.getExtensionManager<TestResource, TestConfig>('multi.priority', 'MultiPriorityTest');
      
      // Register multiple default factories with different priorities to trigger the multi-default path
      multiManager.registerFactory('LowPriority', TestFactoryA, { isDefault: true, priority: 5 });
      multiManager.registerFactory('HighPriority', TestFactoryB, { isDefault: true, priority: 15 });
    ...
    it('should return single default without complex logic', () => {
      const singleManager = ExtensionManager.getExtensionManager<TestResource, TestConfig>('single.default', 'SingleDefaultTest');
      
      // Register only one default factory - should take the fast path
      singleManager.registerFactory('OnlyDefault', TestFactoryC, { isDefault: true, priority: 10 });
    ...
    it('should handle multiple defaults with sorting', () => {
      const sortManager = ExtensionManager.getExtensionManager<TestResource, TestConfig>('sort.test', 'SortTest');
      
      // Register multiple factories to ensure we go through the sorting path
      sortManager.registerFactory('Priority1', TestFactoryA, { isDefault: true, priority: 1 });
      sortManager.registerFactory('Priority10', TestFactoryB, { isDefault: true, priority: 10 });
      sortManager.registerFactory('Priority5', TestFactoryC, { isDefault: true, priority: 5 });
      ...
      expect(result).toBeDefined();
      expect(['Priority1', 'Priority10', 'Priority5']).toContain(result![1]);
    });

    it('should handle factories with undefined priorities in sorting', () => {
      const undefinedManager = ExtensionManager.getExtensionManager<TestResource, TestConfig>('undefined.priority', 'UndefinedPriorityTest');
      
      // Create factory instances with truly undefined priorities
      class NoPriorityFactory implements ResourceFactory<TestResource, TestConfig> {
        ...
        validateConfig(_config: TestConfig): { valid: boolean; errors: string[] } {
      ...
      class WithPriorityFactory implements ResourceFactory<TestResource, TestConfig> {
    ...
    it('should handle equal priorities and undefined priority in best factory selection', () => {
      const equalPriorityManager = ExtensionManager.getExtensionManager<TestResource, TestConfig>('equal.priority', 'EqualPriorityTest');
      
      // Create factories with the same priority to test sorting stability
      class EqualPriorityFactory1 implements ResourceFactory<TestResource, TestConfig> {
      ...
      class UndefinedPriorityWinnerFactory implements ResourceFactory<TestResource, TestConfig> {
      ...
      class EqualPriorityFactory2 implements ResourceFactory<TestResource, TestConfig> {
      ...
      equalPriorityManager.registerFactoryInstance(new EqualPriorityFactory1());
      equalPriorityManager.registerFactoryInstance(new UndefinedPriorityWinnerFactory());
      equalPriorityManager.registerFactoryInstance(new EqualPriorityFactory2());
      ...
      expect(['EqualPriorityFactory1', 'EqualPriorityFactory2']).toContain(result![1]);
    });

    it('should handle undefined priority as the best factory selection', () => {
      const undefinedBestManager = ExtensionManager.getExtensionManager<TestResource, TestConfig>('undefined.best', 'UndefinedBestTest');
      ...
      class OnlyUndefinedFactory implements ResourceFactory<TestResource, TestConfig> {
      ...
      class AnotherUndefinedFactory implements ResourceFactory<TestResource, TestConfig> {
        ...
        async create(config?: TestConfig): Promise<TestResource> {
          return { name: 'another-undefined', config: config ?? null };
        }
      ...
      undefinedBestManager.registerFactoryInstance(new OnlyUndefinedFactory());
      undefinedBestManager.registerFactoryInstance(new AnotherUndefinedFactory());
      ...
      expect(result).toBeDefined();
      expect(['OnlyUndefinedFactory', 'AnotherUndefinedFactory']).toContain(result![1]);
    });
  });
});