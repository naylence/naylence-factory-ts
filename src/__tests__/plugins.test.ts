...
import { ConventionPluginResolver, _internal as resolverInternal } from '../plugin-resolver.js';
import {
  loadPlugins,
  loadPluginsFromEnv,
...
describe('Plugin loading', () => {
  ...
  it('loads plugins via resolver and registers factories without side effects', async () => {
    ...
    const [bestFactory, bestType] = ExtensionManager.getBestGlobalDefaultFactory(BASE_TYPE) ?? [];
  ...
  it('falls back to process cwd resolution when direct import fails', async () => {
    ...
    const pluginModule = { default: { register } } as Record<string, unknown>;
    const error = Object.assign(
      new Error("Cannot find package 'naylence-runtime' imported from test"),
    ...
    pluginsInternal.setResolveFromCwd(async () => 'file:///resolved/plugin.mjs');
    ...
    expect(importer).toHaveBeenNthCalledWith(2, 'file:///resolved/plugin.mjs');
