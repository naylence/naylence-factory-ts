...
let customResolveFromCwd: ((specifier: string) => Promise<string | null>) | null = null;
...
function getDynamicImporter(): DynamicImporter {
  ...
  return (specifier: string) => import(specifier) as Promise<Record<string, unknown>>;
}
...
function isBarePackageSpecifier(value: string): boolean {
  ...
  if (value.startsWith('.') || value.startsWith('/') || value.startsWith('node:') || value.startsWith('file:') || value.startsWith('data:')) {
    return false;
  }
...
async function resolveFromProcessCwd(specifier: string): Promise<string | null> {
  ...
  if (!cachedNodeRequire) {
    try {
      const [{ createRequire }, urlModule] = await Promise.all([
        import('node:module') as Promise<{ createRequire: (filename: string | URL) => NodeRequireLike }>,
        import('node:url') as Promise<{ pathToFileURL: (path: string) => URL }>,
      ]);
  ...
  try {
    ...
    if (!cachedPathToFileURL) {
      const { pathToFileURL } = await import('node:url') as { pathToFileURL: (path: string) => URL };
    ...
    if (/dist[\\/]+cjs[\\/]+/u.test(resolved)) {
      ...
      try {
        const { access } = await import('node:fs/promises') as { access: (path: string) => Promise<void> };
    ...
    if (process?.env?.FAME_PLUGINS_DEBUG) {
      console.debug('[plugins] cwd resolution failed', { specifier, error });
    }
    return null;
  }
}

async function importPluginModule(spec: PluginSpec): Promise<Record<string, unknown>> {
...
export async function loadPluginsFromEnv(envKey = 'FAME_PLUGINS'): Promise<void> {
...
export async function loadPluginsFromSpecs(specs: Iterable<PluginSpec>): Promise<void> {
...
export const _internal = {
  ...
  setResolveFromCwd(resolver: ((specifier: string) => Promise<string | null>) | null): void {
