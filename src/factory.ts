 ...
 */
export abstract class AbstractResourceFactory<T = unknown, C = unknown> implements ResourceFactory<T, C> {
  public abstract readonly type: string;
  public readonly isDefault: boolean = false;
  public readonly priority: number = 0;
  
  public abstract create(config?: C | Record<string, unknown> | null, ...kwargs: unknown[]): Promise<T>;
}

/**
 * Type constraint for factory constructor functions
 */
export type FactoryConstructor<T = unknown, C = unknown> = new (...args: unknown[]) => ResourceFactory<T, C>;
